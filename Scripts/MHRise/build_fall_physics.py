...
def main() -> int:
    ...
    if landing:
        ...
        L += [
            ...
            "✅ **项目已实现这个重设**（`PlayAerialLandingVisual()` 认领落地表现时按方向写 "
            "`AerialLandingHorizontalSpeed = 337` + 同次 CMC 更新末的一次性校正）："
            "PIE 实测两档入速（约 900 / 约 300 cm/s）写后均为 337.0。"
            "真值侧「`148` 首帧」与项目哪一个采样点对应（首采样帧读 900、次帧才是 337）仍待定义，"
            "见 `docs/using/空中下落实现缺口.md` §2 与 `空回P0-P1完整修复方案.md` 的 24:00 审计（P1-1）。",
        ]
    text = "\n".join(L) + "\n"
