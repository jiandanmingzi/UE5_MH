...
NAME_CONFLICTS: dict[str, tuple[str, str]] = {
    "188": ("A", "资产译名是「操虫斩**结束**」，表名写「未击中」—— `Over` 不等于「未击中」；"
                  "录制里 `188` 的后继 58/60 是 `182 落地`，**看起来**像未命中，但那是推断不是命名证据。"),
    "189": ("A", "资产译名「操虫斩命中」，表名写「操虫斩/觉虫击命中进入舞踏」。"
                  "**表名更准**：录制里 `189` 的前驱同时有 `187`（操虫斩，86 段）和 `199`（觉虫击，7 段），"
                  "确实是两个入口共用的那一段。资产只覆盖了操虫斩那一半。"),
    "196": ("A", "资产译名「绝虫击**姿势**」，表名写「突进前摇」。"
                  "且这段在录制里**恒为 1 个样本**、从不构成有效段 —— 两种说法都无法用录制证伪。"),
    "206": ("A", "表名是**两句拼起来**的（「猎虫滑翔命中」+「以及觉虫击未命中的下坠段」），资产只覆盖前半。"
                  "这与链路相符：`206` 确实是**觉虫击飞满全程后的下坠段**（`199`→`206`，28/28 飞满 44 帧）。"),
    "450": ("A", "资产译名「**小**硬直起身（坐地）」，表名写「受伤坐下后的站起」—— 表里没有了「小」这一档。"),
    "451": ("A", "资产译名「**小**硬直起身（趴地）」，同上。"),
    "454": ("B", "资产译名「风压**硬直**」——「硬直」与「恢复」是同一件事的两端。"),
    "455": ("B", "资产译名「**龙吼**硬直」，表名写「吼叫恢复」—— 连用词都不一样。"),
    "456": ("B", "资产译名「地震**硬直**」。"),
    "154": ("B", "资产译名就是「舞踏」，表名多了「突进回旋斩进入」这层入口描述。"),
    "169": ("B", "资产译名「猎虫回归**起跳动作**」，表名写「猎虫回归后段」—— 一个说起跳、一个说后段。"),
...
# （表「操虫斩/觉虫击命中进入舞踏」vs 资产 `CaoChongZhan_Hit`=操虫斩命中）**一直没被列出来**。
