...
### M4.6 入口 Section 与分段位移（代码验证；E4.3 已完成）
...
**已完成的原生验证（2026-09-04，2026-09-05 两次复验）：** Development Editor 编译通过；`MHGZ.M4` 自动化 26/26 通过，其中 `MHGZ.M4.6.Attack.EntrySection.Precedence` 验证 `TransitionID → SourceState → Default` 优先级，`MHGZ.M4.6.Attack.EntrySection.FallbackAndReject` 验证无映射从 Montage 开头开始、无效映射在播放前拒绝；`MHGZ.M4.Serialization.LegacyPropertiesRemoved` 断言攻击、闪避、Combo 与 Dummy 的旧反射属性均不存在，并确认两个 Dodge Montage 不含已删除的旧 Notify。第二轮清理还删除了 Combo `CoreRedirects`、两个旧 Notify 类及两个 Montage 中共四个实例。`-run=DataValidation` 冷启动加载 519 个资产，受影响的 GA、Combo、Montage 均可加载，未报告 Missing Property/Class 或蓝图编译错误；仅保留已知 G-002：`DA_TrainingDummy` 不含恰好 Red/White/Orange 三个 Hitzone。

**E4.3 资产与 PIE 验收（2026-09-09）：** 最终地面 GA、Montage 与 Combo 转移已经接线；用户已在 PIE 逐项验证当前全部地面招式。Dodge 入口混合段随后按 60fps 三帧静止姿势补齐，所有 Section、无敌窗口、Root Motion Phase 与 Handoff 通知已整体校验；Development Editor 完整编译通过，`MHGZ.M4.Dodge` 4/4 通过。E4.3 已签收；木桩三色 Hitzone 的 G-002 仍单独属于 E5.1。

### M4.7 近战虫印与虫印斩首个可玩接线（进行中）

`GA_IG_MarkSlash` 的最终数据型子类必须以 `UMHGZMarkSlashAbility` 为父类，并配置为正常的近战 `AttackSegment`。物理 `Input.Modifier.RT` 始终立即解析为一次 `Input.Weapon.RT`，并持续保留为修饰键直至松开；具体是否进入虫印斩仍由 Combo 的持刀、地面和窗口规则决定。只有某个 Segment 的 Damage GE 已成功提交到有效 Hitzone 后，首个真实 `HitResult` 才会建立/替换唯一虫印。空挥、无效 Hitzone、伤害提交失败或中断都不得影响旧印。近战来源不生成虫印弹 Actor；远程 `LT+RT` 与近战共享 `URes_InsectGlaive` 的局部附着点、到期、目标失效、卸装和镜像 Tag 生命周期。

**四连印斩基础接线（2026-09-10）：** `GA_IG_SiLianYinZhan` 有四个独立单跳 AttackSegment：第 1、3、4 段为 `IG_FrontTip → Root`，第 2 段为 `IG_RearTip → root`，全部动作值/破坏值为 10。`UMHGZMarkSlashAbility.MarkSegmentIndex=1` 使只有第 2 段的成功伤害提交会建立/替换虫印。`AM_IG_SiLianYinZhan` 逐帧比对 `AS_Unsh_R_HengSaoYinZhan` 尾帧和 `AS_Unsh_R_YinZhan` 后，选定后者源第 53 帧（0.8833s）为交接点：`Entry_From_HengSao` 播放完整横扫过渡后跳 `Core`；`Entry_From_ErLian` 播放 `YinZhan` 0～53 帧后跳 `Core`；`Entry_From_ShangLaoFeiShenHouChe` 先播同 `YinZhan` 第 0 帧的 60fps 三帧静止段，再进入 `Entry_From_ErLian`；`Core` 从第 53 帧开始。物理时间轴顺序固定为 `Entry_From_HengSao → Entry_From_ShangLaoFeiShenHouChe → Entry_From_ErLian → Core`，入口跳转不受此线性排序影响。GA 已按这三种入口映射 `HengSao`、`ErLian`、`ShangLao/FeiShenYueRu/HouChe` 的 `YB` 窗口派生，五条边均阻断 Hitstun。Montage 有 `Attack` / `Combo` / `Dodge` 轨道；四个精确 AttackCollision 窗口仍待逐帧配置。

**已完成的原生/资产验证（2026-09-10）：** Development Editor 编译通过；`MHGZ.M4.7.Mark.MeleeLifecycleKeepsUniqueResourceOwnedMark` 1/1 验证近战无投射物虫印、无效命中保留旧印、有效命中替换唯一印及显式清理释放 Tag。`MHGZ.M4.3.Input` 8/8 回归通过。`DA_IG_InputProfile` 仅保留一条裸 `Input.Modifier.RT → Input.Weapon.RT` chord：无 `RequiredContextTags`、`OnPress`、精确修饰键且消费触发键；物理 RT 仍持续保留至松开。虫印资产生成完成后，重建用 Commandlet 源码已移除，最终 `.uasset` 为唯一配置来源。逐帧骨骼姿势比对选定横扫转虫印第 54 帧与虫印第 37 帧为交接点。`Entry_From_HengSao` 仅含三帧静止段与横扫过渡后直跳共享 `Core`；`Entry_From_Idle` 仅含三帧静止段与虫印 0～37 帧后跳转同一 `Core`。Montage 固定空通知轨道 `Attack` / `Combo` / `Dodge`。同时验证 `GA_IG_ChongYin` 的 `RearTip → root` 单段 10/10 配置、`RT_ChongYin` / `HengSao_RT_ChongYin` 与 `DA_IG_Combat → PS_IG_MarkYellowPowder` 引用。窗口实例仍由用户在 Montage 内配置。四连印斩、突进回旋斩、反击窗口/舞踏弹跳仍是本阶段后续工作，尚不可标记 M4.7 完成。

**突进回旋斩动作与派生接线（2026-09-11）：** `GA_IG_TuJinHuiXuanZhan` 保留三段 `ContactOnly` 单跳 AttackSegment，均复制突刺第 0 段的 `IG_FrontTip` 武器 Sweep 与伤害配置，逻辑输入为 `Input.Weapon.YB`。`AM_IG_TuJinHuiXuan` 的统一 `Entry` 为 `AS_Unsh_IdleTuJinHuiXuan` 第 0 帧的三帧静止段，随后完整播放该 Idle 入口；逐帧骨骼局部姿势比对其尾帧与 `AS_Unsh_TuJinHuiXuan` 后，选定后者第 35 帧（0.5833s / 共 277 帧）开始 `Core`，前 35 帧未采用。`Entry` 尾帧（Idle 0.5000s）与 `Core` 首帧（回旋 0.5833s）之间现插入一个 60fps 八帧、逐骨骼平移/旋转/缩放的 C² 五次曲线桥；它属于 `Entry` 的末尾，随后才进入不变的 `Core`。两端各 12 个实际 60fps 帧的姿势趋势审计得到合并 RMS 速度/加速度 p100 上限 9.132/4.114；2～7 帧均超出至少一个上限，八帧是首次同时满足的结果。桥接扩展了 0.0833s，`Core` 和其后的 Section/Notify 已等量后移，入口和 Core 源帧未改变。Montage 已建立空 `Attack` / `Combo` / `Dodge` 通知轨道，三个精确段窗口仍由用户配置。`DA_IG_Combo` 的 Idle/突刺/袈裟前向 YB 起手，以及后续 Y 上捞、B 袈裟、后+Y 上捞、后+B 后撤均已接线。Y 上捞精确映射至飞身跃入入口；B 袈裟和后+Y 上捞各新增一个三帧静态入口，再衔接既有突刺/横扫入口。`AM_IG_ShangLao` 的终止 `Core` 曾错误路由回 `Entry_From_TuJinHuiXuanZhan_Back`，继而顺序进入横扫入口而循环；已改为 `Next=None`，其余 Entry → Core 路由不变。`AM_IG_JiaSha` 也曾以 `Core → Entry_From_TuJinHuiXuanZhan → Entry_From_TuCi → Core` 循环，现同样以 `Core.Next=None` 终止。随后以 `DA_IG_Combo` 全部 45 条边及 `/Game/Blueprints/Ability` 的 18 个 GA 为范围复审：22 个被引用 Montage 的入口引用均有效、全部 Section 路径均终止，无循环或断链。后+Y 边的 `MaxCorrectionAngle=180` 冻结进本次激活上下文，仅该边可在 Montage 起播前直接转向身后；其他上捞仍继承 GA 的常规角度。无需复制袈裟或上捞 GA。一次性生成器已删除；最终 Development Editor 编译通过，`MHGZ.M4` 34/34 通过（含转向角覆盖回归），Data Validation 冷启动未报告本次资产问题；全项目仅保留既有 `DA_TrainingDummy` 三色 Hitzone G-002。

**突进回旋斩反击入口（2026-09-11）：** `GA_IG_TuJinHuiXuanZhan` 现以 `UMHGZAdvancingCounterAbility` 为父类，`AM_IG_TuJinHuiXuan` 有独立空 `Counter` 轨道。配置 `IG Advancing Counter Window` 后，Notify 以 `Mesh + MontageInstanceID` 解析当前 `ActionToken`，并以窗口持续时间作为 TTL 向 Character 的 `IncomingHitResolver` 注册 Token；End、Ability End 和反击成功都幂等注销。只有 `bCounterable` 的真实 IncomingHit 能被消费；首次成功反击立即关闭全部攻击碰撞段、增加 `URes_InsectGlaive` 的 `AdvancingCounter` 舞踏层并结束回旋斩，同一激活不能再次消费。`URes_InsectGlaive` 现公开层数、来源、配置倍率和状态 Delegate，叠层遵守 `MaxDanceStacks` 上限。此处没有用 `LaunchCharacter` 或临时 Root Motion 伪造弹跳：`BallisticVault`、空中 Tag/许可重置和落地清层仍属于 M5 的唯一位移所有权。Development Editor 编译通过，`MHGZ.M4` 34/34 通过；Data Validation 561 assets 仅余既有 `DA_TrainingDummy` G-002。

**突进回旋斩按住蓄力（2026-09-11）：** `AM_IG_TuJinHuiXuan` 保留原有 `Entry` 的静止混入、`AS_Unsh_IdleTuJinHuiXuan` 与八帧桥接，并将原 `Core` 的 `AS_Unsh_TuJinHuiXuan` 段拆为：`Entry` 继续播放源第 35～55 帧的下蹲/向后转身，`Charge` 播放源第 55～67 帧，`Core` 从第 67 帧开始向前突进。`Charge` 不循环：该 0.100 秒原始段以 `AnimPlayRate=0.05` 一次性拉长为恰好 2.000 秒，默认路由为 `Entry → Charge → Core → None`；因此根骨骼位移和姿势都是连续慢放，不会每 0.1 秒回到蓄力首帧。`Core` 和其后 5 个 Montage 通知（Attack/Combo/Dodge/Counter）已随之整体后移 1.900 秒，保持原有相对窗口。`GA_IG_TuJinHuiXuanZhan` 使用可复用的 `UMHGZAdvancingChargeAbility`：每个动作只需配置 Entry/Charge/Core Section 与 `ChargeReleaseControls`；本动作设为 Y、B。Router 在同一个 Y+B 激活身份上派发两枚 Completed，而 Ability 在下一 Tick 读取实际物理按键状态，只有 Y、B **均**已松开才会提前改 `Entry → Core` 或从 `Charge` 跳至 `Core`。点按仍完整播放下蹲后直入出招；持住则完整播放 2 秒蓄力。该层可复用于单一蓄力等级、固定时长的“入口—蓄力—出招”动作；多档蓄力、耐力结算或持续特效需在其上另接专属层。`MHGZ.M4.7.Input.ChordMultiReleaseUsesOneActivationIdentity` 覆盖两种先松开情况。
...
33. **预输入缓冲**：ComboData 的 `PreInputLifetime`（默认 0.5s）控制一条 Coordinator 缓冲；Input Router 先解析完整逻辑输入与摇杆快照，只有除 Combo/Dodge 接受窗口外均满足的指令才可写入。(a) ComboWindow 开前按 Y→窗口开启后按正常连招表复验并发出。(b) 超过有效期→无响应。(c) 连续按 Y→B→Y→后输入覆盖前输入，仅保留最后的 Y。(d) 攻击中 DodgeAccept 开前按 A→开启后经原 Dodge GA 路径发出；新动作确认、动作结束、打断或 ResetCombo 均清空缓冲。
...
39. **GameplayEvent 受击连打**：(a) 受击→GA_HitReaction 实例1 激活。(b) 连打中再次受击→实例2 激活（InstancedPerExecution）→实例1 被打断。(c) 实例2 播完→Hitstun Tag 移除
    - 当前可验收范围：持刀状态下，木桩火圈从角色的前/后/左/右命中时分别播放 `Little_Forward/Back/Left/Right`；受击会结束正在进行的攻击、翻滚或收刀并锁定移动。收刀状态不播受击、不打断动作，但仍承受伤害。中/大硬直等待受击资产后再扩展。
