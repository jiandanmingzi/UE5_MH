...
> **状态：冻结设计，M0～M2 与 E0～E2 已完成。** M2 已解除旧 DataTable/Attack 运行时读取；本文规定的旧虫棍动作包现进入 E3 删除重建窗口。2026-08-11 的 UE 5.6 Asset Registry 审计确认旧链只有一个最小 Combo 节点、两个无自定义 Notify 的 Montage 和两个试验 GA，因此不迁移其数据。
