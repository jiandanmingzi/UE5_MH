...
3. M4.3 是 M4.4 的前置。M4.4 完成并由 E4.2 接线后，必须通过 M4.5。其后先完成 M4.2.1 的普通移动档位重搜 / Blend 验收，才能开始 M4.6。M4.6 的 Entry Section 合同是所有拆分攻击资产的前置。
...
| M4.2.1 普通移动档位重搜 / Blend | M4.5；M4.2 的 Stop 生命周期保持已签收。 | 有效输入中的目标 Family 改变仅触发一次 GaitChange 重搜；Run↔Sprint 等改挡不被 Continuing Loop 延迟；两个普通移动 MM 节点均按 `Blend Time=0.06s`、`Use Inertial Blend=false`、`Max Active Blends=1` 验收。 | **当前阶段，未开始。** |
...
**当前阶段：M4.2.1 普通移动档位重搜 / Blend（运行时代码与 PIE，未开始）。** M4.1/E4.1、M4.2～M4.5 与 E4.2 均已签收；最近 PIE 与 RuntimeTelemetry 已确认动作退出的 Exit→Move/Stop/Idle 合同。最新 Loop→Sprint Telemetry 证明：RB 会同帧更新目标档位，但 Continuing Loop 可延迟目标候选，因此此项作为独立普通移动修正切片先行处理。

当前唯一允许的下一步是 M4.2.1：实现并验收一次性 `GaitChange` 重搜合同；目标 Family 改变时仅撤销一次源 Loop 的 Continuing 优先权，以新 `MMMoveGaitQuery` 立即新搜，并使用 `Blend Time=0.06s`、`Use Inertial Blend=false`、`Max Active Blends=1` 完成视觉交接。不得重置 Stop 生命周期、不得每帧强制重搜、不得以 `DesiredSpeed` 或输入死区代替该边沿。
