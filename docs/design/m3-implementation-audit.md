...
- `FWeaponChordDefinition` 新增通用 Required/Blocked ContextTags。收刀 RT 单成员 Chord 可立即触发；持刀 LT+RT 支持任意顺序；收刀 RB 不产生 Router 离散快照，仅由 Character 的 0.1 秒长按逻辑处理；持刀地面 RB 的 `Input.Sheathe` Chord 则要求 `Unsheathed+Grounded`。
...
- `Automation RunTests MHGZ.M3`：10/10 通过，命令行退出码 0；其中 `MHGZ.M3.Kinsect.ProjectileMovementAndRecall` 直接按正式 `ProjectileMovementComponent -> AKinsect::Tick` 顺序验证发射、位移、手动召回和重新挂载。M3 harness 是无 WorldContext 的临时 World，因此该回归不得调用 `UWorld::Tick`。
...
- 使用正式 Development DLL 再运行 `Automation RunTests MHGZ.M3`：10/10 通过，命令行退出码 0。
