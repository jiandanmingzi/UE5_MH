...
> **2026-09-23 空回 P0/P1 后续状态（更新后证据优先于本文旧状态描述）：** 实际空回 GA/蒙太奇/JumpForce/CMC/Host 下落链的无碰撞回归已通过，约 900 cm/s 水平速度连续；场景录制的淡出时速度突降需要新增的 CMC HandleImpact 命中记录才能归因。落地 337 cm/s 已增加 CMC 更新末的一次性校正并通过自动化，仍待 PIE 两档入速复验。落地动画结束后的 MM 姿态出口按用户要求暂缓。详细证据与执行步骤见 [空回 P0/P1 完整修复方案](空回P0-P1完整修复方案.md) 与 [空回坠落顿挫审查](空回坠落顿挫与项目现状审查.md)。
...
> 上表“空中受击表现”和“反击触发延迟”的历史记录及 `20260924-164759` PIE 覆盖情况见 `docs/using/项目已知的不重要问题.md`。该录制没有空中受击样本，也缺少火环命中的精确时刻，不能据此关闭这两项。

## 8. 阶段 D — 六个空中招式（**第 1 招已实现，待 PIE**）

**「五个」是旧数字**：2026-09-22 核对了冻结 M5 工作列表后发现**空中回避（MHR id `137`）两头都没挂** ——
设计有（`actions.md:400` 的 `GA_AirDodge` + `CantDodge` 预算模型）、真值测完（`下落物理.md:17,53-70`）、
tag 已定义（`DefaultGameplayTags.ini:64`），但冻结工作列表没有它、本节也没有它、§12 边界表同样没提它，
只有 `空中下落实现缺口.md:32` 把它记为「**最大的缺口**」。**用户 2026-09-22 拍板：纳入 M5 阶段 D 并先做它。**

### 8.1 空中回避 —— **已实现**（2026-09-22，待 PIE）
...
- **不做落地重设水平速度**（`空中下落实现缺口.md` §2：落地把水平速度重设为 337 cm/s，实测 `148` 首帧恒 337±6、与入速无关）。它改**所有**既有落地（撑杆跳、舞踏）的行为，要自己的 A/B 回归 ⇒ 单独一轮。**空中回避那一轮刻意没带上它。**
