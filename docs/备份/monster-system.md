...
| 不注册 AttributeSet | 木桩不需要血量，后续需要时再挂 |
...
| 层 | 机制 | 内容 |
|----|------|------|
| 帧级同步 | GameplayCue（Execute） | 命中火花、斩击/打击音效（按 Hitzone 物理材质选） |
| 数值显示 | UWidgetComponent 浮空文字 | 伤害数字（暴击时放大+变色），1.5s 淡出 |
| 可选 | 受击 Montage | 木桩轻微抖动（`GA_HitReaction`），当前版本可略 |

---

## 伤害数字（UMHGZDamageNumberComponent）

| 成员 | 类型 | 说明 |
|------|------|------|
| DamageValue | int32 | 显示数值 |
| bIsCritical | bool | 是否暴击（放大 1.5x + 黄色） |
| Lifetime | float | 存活时间（默认 1.5s） |
| FloatVelocity | FVector | 上浮速度（默认 Z+80/s） |

`GameplayCue.Hit.DamageNumber` 触发时从对象池取 Widget，设置数值/暴击标记，1.5s 后回收。
...
└── MHGZDamageNumberComponent.h/cpp     ← 伤害数字浮空文字
